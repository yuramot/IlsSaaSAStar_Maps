...
uses
  ...
  AStar64.DynImport in 'VisualTester\AStar64.DynImport.pas',
...
begin
  try
    ...
    begin
      ...
      Writeln('Пример: LandmarkPathCalc.exe 1 -t:3 -timeout:900 -calc:all');
...
//          Account := StrToInt(ParamStr(I));
  ...
  //  Writeln('ProcessAccs, Timeout='+IntToStr(Timeout));
    ProcessAccs(GPathList, Timeout, RecalcAll);
  except
    on E: Exception do
    begin
      Log('Global error: ' + E.ClassName + ': ' + E.Message);
    end;
  end;
end.
