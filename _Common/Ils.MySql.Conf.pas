...
type
  ...
  TMySQLDatabaseConfig = record
    ...
    constructor Create(
      AHost: string;
      APort: Integer;
      ALogin, APassword, ADatabase, AProtocol: string;
      ALogs: Boolean = True
    ); overload;
    constructor Create(AIni: TIniFile; ALogs: Boolean = True); overload;
...
constructor TMySQLDatabaseConfig.Create(
  AHost: string;
  APort: Integer;
  ALogin, APassword, ADatabase, AProtocol: string;
  ALogs: Boolean = True
...
begin
  ...
  ToLog(Format('IP адрес СУБД => "%s"', [Host]));
  ToLog(Format('IP порт СУБД => "%d"', [Port]));
  ToLog(Format('Логин СУБД => "%s"', [Login]));
  ToLog(Format('Пароль СУБД => "%s"', [Password]));
  ToLog(Format('Протокол СУБД => "%s"', [Protocol]));
  ToLog(Format('СУБД => "%s"', [Database]));
end;

constructor TMySQLDatabaseConfig.Create(AIni: TIniFile; ALogs: Boolean = True);
begin
  Create(
